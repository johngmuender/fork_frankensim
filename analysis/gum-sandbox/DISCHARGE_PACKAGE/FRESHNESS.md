...
- **F5 / K28:** `cargo run --release --bin h22_solve -- all` — the binary has no `all` subcommand (usage: `single|pair|sdiff|twist`, each with required args). Corrected form is the five shipped runs of h22_RESULTS.md (single / pair align / pair repulse / sdiff repulse / twist repulse at m=0.9 → `h22_runs/*_m09.json`), ~24 min total; initially SKIPPED-BUDGET, then EXECUTED post-gauntlet: single 800 / pair align 9 1200 / pair repulse 9 1200 / sdiff repulse 9 150 / twist repulse 9 21 (parameters recovered from the primaries' own protocol blocks), scratch outdir per the non-clobber rule — all five PHYSICS BIT-IDENTICAL to h22_runs/ primaries; the only differing field is the embedded wall-clock seconds (caveat: the crate's 'bit-identical JSON' claim holds modulo that field, which its header comment should state).
...
**All 46 table rows executed fresh on this host on 2026-07-16/17 — INCLUDING K28's corrected five-run form (executed post-gauntlet when credits were restored; physics bit-identical 5/5), so the gauntlet closed with ZERO skips; every physics comparison — 7 golden Merkle roots, 3 run fingerprints, every results-JSON physics field, every PNG, and the full N=192 iteration series — reproduced BIT-IDENTICALLY; the only findings are process-level (two printed-command defects, stale runtime/expectation text, one truncated shipped log, one PYTHONHASHSEED hang hazard that never affects results). Zero physics drift. The package REPRODUCES.**
