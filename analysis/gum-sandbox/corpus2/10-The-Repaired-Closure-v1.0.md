...
## §7 [H4 PENDING] — the box this appendix must carry

> **[H4 PENDING].** The closure's admissibility premise (j ∈ ½ℤ) routes through QUANT/App F.5's consistency part, which consumes the unproven minimal hypothesis **MH** (im ∂ ⊆ A for ∂: π₂(ℝP²) = ℤ → π₁(F_ℓ), plus splitness, plus A-blind exchange). The discharge computation is named, finite, and in flight: evaluate ∂ on π₂(ℝP²)'s generator + the extension class. Outcomes, pre-printed: completion I — benign, this appendix stands as written; II — the printed "ℤ₂ × A" is false but a ℤ₄-type double-valued sector survives, this appendix stands with re-worded topology; **III — only integer j is admissible and the closure system (repaired or not) has no solution: this appendix, and the closure it repairs, would be unsolved together.** Patched when H4 reports.
